...
//A minor version of resetGame used when Pacmanloses a life
void respawn(State& state) {
    state.pacman = initializePacman(state.mainTexture);
    state.ghosts = Enemies(state.mainTexture);
...
void damagePacman(State& state){
    if(state.lives > 1) {
        state.lives -= 1;
        respawn(state); //Reset pacman and ghosts
    }else{
        setGameOver(state); 
    }
}
...
void drawScore(sf::RenderWindow& window, const State& state) {
    sf::Text scoreText{state.font, "Score:" + std::to_string(state.score), 12};
    ...
    sf::Text highScoreText{state.font, "High Score: " + std::to_string(loadHighScore()), 12};
    highScoreText.setPosition(HIGHSCORE_POSITION);
    window.draw(highScoreText);
}

void drawLives(sf::RenderWindow& window, const State& state) {
    sf::Text livesText{state.font, "Lives: ",12};
    livesText.setPosition(LIFETEXT_POSITION);
    window.draw(livesText);

    sf::Sprite lifeSprite(state.mainTexture);
    sf::IntRect lifeRect(PACMAN_TEXTURE_DOWN1, sf::Vector2i(CHARACTER_SIZE, CHARACTER_SIZE));
