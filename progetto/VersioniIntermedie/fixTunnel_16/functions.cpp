...
std::vector<sf::Vector2i> getPossibileDirectionsInt(const Ghost& ghost) {
    if (ghost.direction == UP) { 
        ...
        return {sf::Vector2i(-1, 0), sf::Vector2i(0, -1), sf::Vector2i(0, 1)};
    } else if (ghost.direction == RIGHT) { 
        return {sf::Vector2i(1, 0), sf::Vector2i(0, -1), sf::Vector2i(0, 1)};
    } else {
        return {sf::Vector2i(0, -1), sf::Vector2i(0, 1), sf::Vector2i(-1, 0), sf::Vector2i(1, 0)}; // No last direction
    }
}
...
sf::Vector2f getNextStepFromMap(const Ghost& ghost, const std::vector<std::vector<int>>& distMap) {
    ...
    std::vector<sf::Vector2i> directions = getPossibileDirectionsInt(ghost); // Get possible directions
