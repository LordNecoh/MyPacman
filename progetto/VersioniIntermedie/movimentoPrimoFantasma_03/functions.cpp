...
void checkTunnel(const sf::Vector2f position, Character& character) {
    ...
    if ( nextPosition.y == 9 && (nextPosition.x == 0 || nextPosition.x == 20)) character.traversingTunnel = true;
}
...
void checkCollisions(State& state){
    ...
    //Checks the tunnel after, so that he moves before teleporting
...
void moveGhost(Ghost& ghost, const sf::Vector2f& target, float deltaTime) {
    int minDistance =  99999; // Arbitrary large number
    std::vector<sf::Vector2f> possibleDirections = getPossibleDirections(ghost);
    for(int i = 0; i< possibleDirections.size(); ++i) {
        sf::Vector2f newDirection = possibleDirections[i];
        if( !canWalk(ghost.sprite.getPosition(), newDirection)) continue; //If the ghost can't walk in that direction, skip it
        if(manhattanDistance(getGridCoords(ghost.sprite.getPosition()), getGridCoords(target)) < minDistance) {
            ghost.direction = newDirection;
            minDistance = manhattanDistance(getGridCoords(ghost.sprite.getPosition()), getGridCoords(target));
        }
    }

    ghost.sprite.move(ghost.direction * 20.0f * deltaTime); 
